  ...
  async createPreference(courseData, userData) {
    try {
      const preferenceData = {
        ...
        external_reference: `course_${courseData.id}_user_${userData.id}_type_${courseData.targetType || 'course'}${courseData.moduleId ? `_module_${courseData.moduleId}` : ''}${courseData.lessonId ? `_lesson_${courseData.lessonId}` : ''}${courseData.eventId ? `_event_${courseData.eventId}` : ''}_${Date.now()}`,
        expires: true,
